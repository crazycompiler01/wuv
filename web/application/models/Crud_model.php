...
class Crud_model extends CI_Model {
	...
	*/
    ...
    function get_featured()
	{
        ...
        $query = $this->db->get('movie', '5');
        $movie =  $query->result();

        foreach ($movie as $m) {
        	
        	//$movie['thumbnail'] = get_thumb_url('movie',$m->movie_id);
        	if (file_exists('assets/global/movie_thumb/' . $m->movie_id . '.jpg'))
            $m->thumbnail = base_url() . 'assets/global/movie_thumb/' . $m->movie_id . '.jpg';
        else
            $m->thumbnail = base_url() . 'assets/global/placeholder.jpg';
        }
